...
const DB_PATH = process.env.PLANNER_DB_PATH || path.join(__dirname, '..', 'data', 'db.json');
const REDIS_KEY = 'weekly-planner:db';

const DEFAULT_DB = { members: [], tasks: [], pushSubscriptions: [] };

// Older saved DBs (local file or an existing Redis blob) predate the
// pushSubscriptions field — backfill it so callers can always rely on it.
function normalize(db) {
  if (!db.pushSubscriptions) db.pushSubscriptions = [];
  return db;
}

// Different Redis marketplace integrations name their env vars differently
// (legacy Vercel KV vs. Upstash native vs. a custom --prefix), so accept any
// of the common pairs rather than betting on one.
const REST_URL =
...
async function readDB() {
  if (hasRedis) {
    ...
    return normalize(db || { ...DEFAULT_DB });
  }
  try {
